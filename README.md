...
For this appliance only, it selects `dry` or `fan_only` in Matter's
`SystemMode` first, then sends Matter's standard `On` command when the A/C is
off. This ordering is necessary because the appliance otherwise accepts the
mode while remaining powered off.
